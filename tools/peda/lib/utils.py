...
@memoized
def format_disasm_code(code, peda, nearby=None):
    """
    ...
    compare_list = peda.arch_helper.get_compare_instrs()
    jump_list = peda.arch_helper.get_jump_instrs()
    call_list = peda.arch_helper.get_call_instrs()
    syscall_list = peda.arch_helper.get_syscall_instrs()
    return_regexes = peda.arch_helper.get_return_regexes()
    ...
    for line in code.splitlines():
        if ":" not in line: # not an assembly line
            ...
            if opcode in colorcodes.keys():
                color = colorcodes[opcode]
