...
import config
from nasm import *
from arch_arm_helper import *
from arch_x86_helper import *
...
class PEDA(object):
    ...
    def execute_redirect(self, gdb_command, silent=False):
        ...
        try:
            ...
            result = 'Exception (%s): %s' % (gdb_command, e)
            if config.Option.get("debug") == "on":
                msg(result, "red")
                traceback.print_exc()
	if silent:
    ...
    def parse_and_eval(self, exp):
        ...
        self.setuparch()
	regs = self.arch_helper.get_register_list()
        for r in regs:
    ...
    def get_arch_helper(self, archname, bits):
        """
        Get architecture specific helper object
	...
	if "arm" in archname:
		return arch_arm_helper(archname, bits)
	if "86" in archname:
		return arch_x86_helper(archname, bits)

	return None

    def setuparch(self):
        """
        Setup architecture helper for debugged program

        Returns:
            - tuple of architecture info (arch (Helper Object), bits (Int))
        """
	if self.arch_helper == None:
	        arch = "unknown"
	        bits = 32
		...
		if out == []:
	        ...
	        for line in out:
	            if "file type" in line:
	                arch_name = line.split()[-1][:-1]
        		if "64" in arch_name:
				bits = 64
	                break
	        self.arch_helper = self.get_arch_helper(arch_name,bits)
		self.bits = bits
		self.arch_name = arch_name
		for gdb_setup_cmd in self.arch_helper.get_gdb_setup_cmds():
		        self.execute(gdb_setup_cmd)

	return

    def getbits(self):
	"""
	Get bitness of debugged program
    ...
    def assemble(self, asmcode, bits=None):
        ...
        Returns:
            - bin code (raw bytes)
        """
        return Nasm.assemble(asmcode, self.getbits())
    ...
    def get_function_args(self, argc=None):
        ...
        self.setuparch()
        if "i386" in self.arch_name:
            args = self._get_function_args_32(code, argc)
        if "64" in self.arch_name:
    ...
    def stepuntil(self, inst, mapname=None, depth=None):
        ...
        Args:
            - insts: list of instructions to reach, regular expression forms separated by ';' (String)
        ...
        while True:
            ...
            found = 0
	    i = inst.replace(";", "|")
	    #print "Pattern is [%s]" % i
            if i != "" and re.match(i.strip(), code.strip()):
            ...
            if found != 0:
                break
            exc_output = self.execute_redirect("stepi", silent=True)
            if exc_output and "Exception" in exc_output:
		return (None, current_instruction.strip())
                break
    ...
    def get_cpuflags(self):
        """
        Get flags value from CPU register
	...
	flags_reg = self.arch_helper.get_flags_register_name()
	return self.arch_helper.get_cpuflags(self.getreg(flags_reg))

    def set_cpuflags(self, flagname, value=True):
        """
        ...
        cpuflags = self.get_cpuflags()
        if not cpuflags:
            return False

	flags_reg = self.arch_helper.get_flags_register_name()
	reg_cpuflags = self.getreg(flags_reg)
	cmd = self.arch_helper.get_cpuflags_update_cmd(flagname, value, reg_cpuflags)
	...
	if "arm" in self.arch_name:
		operand = inst.split(":")[1].split()[1]
		target = self.parse_and_eval(operand)
		print "Target is %s" % target
		return to_int(target)

        # this regex includes x86_64 RIP relative address reference
	...
	#print "Next_addr evaluated to %s" % hex(next_addr)

	condition_met = self.arch_helper.test_conditional(opcode, flags)
	if condition_met:
		return next_addr
	else:
		return None
    ...
    def examine_mem_value(self, value):
        ...
        if self.is_writable(value): # writable data address
            out = examine_data(value, self.bits)
            if out:
    ...
    def search_asm(self, start, end, asmcode, rop=0):
		...
		print "--------"
	        ...
	        self.setuparch()
                ...
                for reg in self.arch_helper.get_register_list():
        ...
        for machine_code in buildcode():
            ...
            if rop and 'ret' not in asmcode:    ###TODO
...
class PEDACmd(object):
	...
	pedaobj = peda
        msg(format_disasm_code(code, peda=pedaobj))
    ...
    def nearpc(self, *arg):
        ...
        if code:
            msg(format_disasm_code(code, self, address))
        else:
    ...
    def deactive(self, *arg):
        ...
        tmpfd.write("\n".join([
                "commands $bpnum",
                "silent",
                "set %s = 0" % peda.arch_helper.get_returnval_expr(),
                "return",
                "continue",
                "end"]))
        tmpfd.flush()
    ...
    def unptrace(self, *arg):
        ...
        # cannot deactive vi plt entry, try syscall method
            ...
            tmpfd = tmpfile()
            if "i386" in peda.arch_name:   ###TODO
                tmpfd.write("\n".join([
                    "commands $bpnum",
                    "silent",
                    "if (*(int*)($esp+4) == 0)",
    ...
    def nextcall(self, *arg):
        ...
        call_instrs = peda.arch_helper.get_call_instrs()
        call_list = ";".join(call_instrs)

        if keyword:
            return self.stepuntil("%s.*%s" % (call_list,keyword), mapname)
        else:
            return self.stepuntil("%s" % (call_list), mapname)
        

    def nextjmp(self, *arg):
        """
        Step until next jump instruction in specific memory range
        Usage:
            MYNAME [keyword] [mapname1,mapname2]
        """
        (keyword, mapname) = normalize_argv(arg, 2)

	#TODO shouldn't generate this each time
        jump_instrs = [ re.escape(i) + "[\t ]+" for i in peda.arch_helper.get_jump_instrs() ]
        jump_list = ";".join(jump_instrs)
        if keyword:
            self.stepuntil("%s.*%s" % (jump_list,keyword), mapname)
        else:
            self.stepuntil("%s" % (jump_list), mapname)
        return
    ...
    def tracecall(self, *arg):
        ...
        while True:
            call_instrs = peda.arch_helper.get_call_instrs()
            result = peda.stepuntil(";".join(call_instrs), mapname, prev_depth)
            if result is None:
                break
            ...
            if matched:
		pedaobj = peda
                code = format_disasm_code(code, peda=pedaobj)
    ...
    def traceinst(self, *arg):
        ...
        peda.save_user_command("hook-stop") # disable hook-stop to speedup
        msg("Tracing instructions matching '%s', Ctrl-C to stop..." % (instlist) )
        prev_depth = peda.backtrace_depth(peda.getreg("sp"))
        logname = config.Option.get("tracelog")
        logname = logname.replace("#FILENAME#", filename)
        logfd = open(logname, "w")
        ...
        p = re.compile(".*?:\s*[^ \t]*\s*([^,]*),[ \t]*([^;,]*),?[ \t]*([^;,]*)")
        while count:
            result = peda.stepuntil(instlist, mapname, prev_depth)
            (call_depth, code) = result
            if call_depth == None:
                break
            ...
            # special case for conditional jump instrs
            prev_code = ""
            condjump_instrs = peda.arch_helper.get_condjump_instrs()

            if code.split(":")[1].split()[0] in condjump_instrs:
            ...
            text = "%s%s%s" % (" "*(prev_depth-1), " dep:%02d " % (prev_depth-1), colorize(code.strip()))
            msg(text, teefd=logfd)

            call_instrs = peda.arch_helper.get_call_instrs()
            if code.split(":")[1].split()[0] in call_instrs:
                args = peda.get_function_args()
                if args:
                    for (i, a) in enumerate(args):
                        chain = peda.examine_mem_reference(a)
    ...
    def context_code(self, *arg):
        ...
        if inst: # valid $PC
            ...
            if opcode in peda.arch_helper.get_call_instrs():
                text += peda.disassemble_around(pc, count)
                msg(format_disasm_code(text, peda, pc))
                self.dumpargs()
            # stopped at jump
            elif opcode in peda.arch_helper.get_jump_instrs():
                ...
                if jumpto: # JUMP is taken
                    ...
                    for (idx, line) in enumerate(code):
                        ...
                        if idx <= pc_idx:
                            ...
                            text += " | %s\n" % line.strip()
                    text = format_disasm_code(text,peda, pc) + "\n"
                    text += " |->"
                    ...
                    text += format_disasm_code(peda.disassemble_around(pc, count), peda, pc)
    ...
    def cpuflags(self, *arg):
        """
        Display/set/clear value of CPU flags register 
        Usage:
            MYNAME
        ...
        if "86" in peda.arch_name:
	        FLAGS = ["CF", "PF", "AF", "ZF", "SF", "TF", "IF", "DF", "OF"]
	        FLAGS_TEXT = ["Carry", "Parity", "Adjust", "Zero", "Sign", "Trap",
	                        "Interrupt", "Direction", "Overflow"]

        if "arm" in peda.arch_name:
	        FLAGS = ["N", "Z", "C", "V", "Q", "I", "F", "T", "M"]
	        FLAGS_TEXT = ["Negative", "Zero", "Carry", "Overflow", "Saturation", "IRQ-mask", "FIQ-mask",
	                        "Thumb", "CPU-mode"]
        ...
        if option is None: # display flags
            flags = peda.get_cpuflags()
            text = ""
	    ...
	    if "86" in peda.arch_name:
	            flags = peda.getreg("eflags")
	            msg("%s: 0x%x (%s)" % (green("EFLAGS"), flags, text.strip()))

	    if "arm" in peda.arch_name:
	            flags = peda.getreg("cpsr")
	            msg("%s: 0x%x (%s)" % (green("CPSR"), flags, text.strip()))


        if option == "set":
            peda.set_cpuflags(flagname.lower())

        if option == "clear":
            peda.set_cpuflags(flagname, False)

        return
    cpuflags.options = ["set", "clear"]
    ...
    def xinfo(self, *arg):
        ...
        if str(address).startswith("r"):
            ...
            if regname is None:
#		if "86" in peda.arch_name:
#			regbits = REGISTERS_X86[bits]
#		if "arm" in peda.arch_name:
#			regbits = REGISTERS_ARM[bits]
		regbits = peda.arch_helper.get_register_list(peda.bits)
                for r in regbits:
                    if r in regs:
            ...
            if text:
                msg(text.strip())
            if regname is None or "eflags" in regname or "cpsr" in regname:
                self.cpuflags()
            return
